...
let rec translate (Ast.Program procedures) = Ast.Program (List.map translateProcedure procedures)

and
  translateProcedure (Ast.Procedure {id; formals; typ; body}) = Ast.Procedure {id; formals; typ; body = translateStatement body}

and
  translateStatement statement = match statement with
  | Ast.Block {decls; statements} -> Ast.Block {decls; statements = List.map translateStatement statements}
  ...
  | Ast.Call {rator; rands} -> Ast.Call {rator; rands = List.map translateTerm rands}
...
and
  ...
  | Ast.Id _ -> term
  | Ast.Literal _ -> term
  | Ast.App {rator; rands} -> Ast.App {rator; rands = List.map translateTerm rands}
  | Ast.If {expr; thn; els} -> Ast.If {expr = translateTerm expr; thn = translateTerm thn; els = translateTerm els}
  | Ast.And {left; right} -> Ast.And {left = translateTerm left; right = translateTerm right}
  | Ast.Or {left; right} -> Ast.Or {left = translateTerm left; right = translateTerm right}
  | Ast.Let {decl; body} -> (match decl with
      | Ast.ValBind {bv = Ast.{id; typ}; defn = Ast.Id keep} ->
        translateTerm (substitute keep id body)
      | Ast.ValBind {bv; defn} ->
        Ast.Let {decl; body = translateTerm body}
      | Ast.FunBind {id; formals; typ; body = innerBody} ->
        Ast.Let {decl = Ast.FunBind {id; formals; typ; body = translateTerm innerBody}; body = translateTerm body})
