...
struct
  ...
  type t = {label : Label.t option; operation : Opn.t; comment : string option}

  let toInstruction(lab, opn, comment) : t =
    {label = lab;
     operation = opn;
     comment = comment}
  ...
  let format instr =
    ...
    in
    match instr with
    | {label = None; operation; comment = None} -> fmt "\t%s" opnfmt

    | {label = None; operation; comment = Some comment} ->
      let tabs = alignComment (Some(String.length opnfmt))
      in
      fmt "\t%s%s# %s" opnfmt tabs comment

    | {label = Some l; operation; comment = None} ->
      let labfmt = Label.format l
      in
      fmt "%s:\t%s" labfmt opnfmt

    | {label = Some l; operation; comment = Some comment} ->
